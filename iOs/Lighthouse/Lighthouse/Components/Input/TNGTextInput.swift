...
class TNGTextInput: CustomViewFromXib, UITextFieldDelegate {
    ...
    var errorPlacehodlerHeight: CGFloat = 0
    ...
    var isEnabled: Bool {
        ...
        set {
            self.textFieldInput.isEnabled = true
        }
    }
    ...
    private func configureUi() {
        ...
        errorPlacehodlerHeight = self.errorPlaceholderHeightConstraint.constant
    ...
    private func showErrorPlaceholder() {
        UIView.animate(withDuration: 0.1) {
            self.errorPlaceholderHeightConstraint.constant = self.errorPlacehodlerHeight
            self.layoutIfNeeded()
        }
